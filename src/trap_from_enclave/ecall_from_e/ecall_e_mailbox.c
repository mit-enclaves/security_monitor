...
#define REGION_IDX(addr) ((1u<<(addr >> log2(SIZE_DRAM / NUM_REGIONS))) & (NUM_REGIONS - 1))
...
inline bool check_buffer_ownership(uintptr_t buff_phys_addr, size_t size_buff, enclave_id_t enclave_id) {
	...
	return owned(buff_phys_addr, enclave_id) && owned(buff_phys_addr + size_buff * 8);
}
...
api_result_t get_attestation_key(uintptr_t phys_addr) {
	...
	memcpy(phys_addr, boot_api_sate.security_monitor_secret_key, size_key);
...
api_result_t accept_message(mailbox_id_t mailbox_id, enclave_id_t expected_sender) {
	...
	if(!sm_globals.cores[csr_read(mhartid)].has_enclave_schedule) {
...
api_result_t read_message(mailbox_id_t mailbox_id, uintptr_t phys_addr) {
	...
	memcpy(phys_addr, mailbox->message, sizeof(uint8_t) * MAILBOX_SIZE);

	// Copy sender measurement into buffer
	memcpy(phys_addr + sizeof(uint8_t) * MAILBOX_SIZE, ((enclave_t *) mailbox->sender)->measurement, sizeof(hash_t));
...
api_result_t send_message(enclave_id_t enclave_id, mailbox_id_t mailbox_id, uintptr_t phys_addr) {
	...
	memcpy(mailbox->message, phys_addr, sizeof(uint8_t) * MAILBOX_SIZE);
