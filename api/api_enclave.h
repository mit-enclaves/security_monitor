...
static inline api_result_t sm_get_attestation_key(uintptr_t phys_addr) {
   return SM_API_CALL(SM_GET_ATTESTATION_KEY, phys_addr, 0, 0, 0, 0, 0);
}
...
static inline api_result_t sm_region_check_owned(region_id_t id) {
   return SM_API_CALL(SM_REGION_CHECK_OWNED, id, 0, 0, 0, 0, 0);
}
...
static inline api_result_t sm_random(uintptr_t random_buf, size_t num_bytes) {
   return SM_API_CALL(SM_RANDOM, random_buf, num_bytes, 0, 0, 0, 0);
}
